...
class CollisionDetector {
    ...
    hasCollidedWithAny(bird, pairs, screenHeight, groundHeight) {
        for (const pair of pairs) {
            //prevent jump over top pipe
            if (bird.y < 0 && (bird.x + 35 >= pair.x)) {
                return true;
            }
            //top pipe
            if (this.hasCollidedWith(bird, pair.x + (pair.width / 2), pair.topHeight / 2, pair.width, pair.topHeight)) {
                return true;
            }
            //bottom pipe
            if (this.hasCollidedWith(bird, pair.x + (pair.width / 2), screenHeight - (groundHeight + pair.bottomHeight / 2), pair.width, pair.bottomHeight)) {
                return true;
            }
        }
        return false;
    ...
    hasCollidedWith(bird, pipeX, pipeY, pipeWidth, pipeHeight) {
        let xDist = Math.abs(bird.x - pipeX);
        let yDist = Math.abs(bird.y - pipeY);
        if (xDist > (pipeWidth / 2 + 35)) {
            return false;
        }
        if (yDist > (pipeHeight / 2 + 35)) {
            return false;
        }
        if (xDist <= (pipeWidth / 2)) {
            return true;
        }
        if (yDist <= (pipeHeight / 2)) {
            return true;
        } // top pipe infin8
        let cornerDistance_sq = (xDist - pipeWidth / 2) ** 2 + (yDist - pipeHeight / 2) ** 2;
